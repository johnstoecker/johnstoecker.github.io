 ...
 *  Modified by Raxa
 */
//menu will take up this much of the screen, rest dedicated to peek view
SLIDE_MENU_WIDTH = .8;
...
Ext.define('Ext.ux.slidenavigation.View', {
    ...
    config: {
         ...
         */
        list: {
            width: window.innerWidth * SLIDE_MENU_WIDTH,
            maxDrag: null,
            itemTpl: '{title}',
           grouped: true,
            items: [{
    ...
    initialize: function() {
       ...
       this.list.select(selectedItemIndex);
        ...
        // item = this.list.getStore().getAt(selectedItemIndex);

        // me._cache[selectedItemIndex] = container.add(Ext.merge({}, me.config.defaults, item.raw));

        // me.doMaskItem(me._cache[selectedItemIndex], true);
...
//            me._cache[selectedItemIndex].addListener('painted', function() {
...
//        }
        ...
        //     me.createSlideButton(me._cache[selectedItemIndex], item.raw.slideButton);
        // }


        //this.moveContainer(this, window.innerWidth * SLIDE_MENU_WIDTH);
    ...
    createSlideButton: function(el, config) {
        var me      = this,
            config  = Ext.merge(me.getSlideButtonDefaults(), Ext.isObject(config) ? config : {}),
            parent  = el.down('titlebar'),
        ...
        if (!Ext.ModelManager.get(model)) {
            Ext.define(model, {
                ...
                config: {
                    idProperty: 'index',
                    fields: [
                        'index', 'title', 'group', 'selected', 'icon',
                        {
                            name: 'order',
                            defaultValue: 1
                        },{
    ...
    createNavigationList: function(store) {
        ...
        var navList =  Ext.create('Ext.dataview.List', Ext.merge({}, listConfig, {
            store: this.store,
            docked: listPosition,
            cls: 'x-slidenavigation-list',
            style: 'position: absolute; top: 0; '+listPosition+': 0; height: 100%;' +
                   'z-index: 2;background: #eee',
            itemTpl: '<span style = "font-size:1.4em"><img src = {icon}>{title}</span>',
            listeners: {
    ...
    createContainer: function() {
        ...
        //if container is open and peek view is on, disable interaction
        container.element.on({
            tap: function(e, node, opts) {
                if(this.isOpened(e, node, opts)){
                    e.stopPropagation();
                }
            }, scope: this
        });
